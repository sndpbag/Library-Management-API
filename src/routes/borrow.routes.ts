...
//     if (!foundBook) return res.status(404).json({ success: false, message: 'Book not found' });
...
//     await borrow.save();
//     res.json({ success: true, message: 'Book borrowed successfully', data: borrow });
//   } catch (error) {
//     res.status(400).json({ success: false, message: 'Error borrowing book', error });
//   }
// });
